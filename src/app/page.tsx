...
export default function Home() {   
  ...
  const toggleSelectAll = () => { 
    const allIds: string[] = []; 
    soList.forEach(so => { allIds.push(so.id); so.penyaluranList.forEach(p => allIds.push(p.id)); }); 
  ...
  if (view === "template") {
    return (
      <TemplateEditor 
        ...
        onLoadHistory={(data: SOData[], periodeH: string) => { 
          setSoList(data); 
          setTanggalPeriode(periodeH); 
          setTanggalTtd(new Date().toISOString().slice(0, 10)); 
          setView("dashboard"); 
        }}
        onReExportHistory={(format: 'EXCEL' | 'PDF', data: SOData[], periodeH: string) => {
