...
export default function Tembusan({ masterData, viewMode, setViewMode, formData, setFormData, triggerDelete }: any) {
  if (viewMode === "list") {
    return (
      <div className="animate-in fade-in slide-in-from-right-4 duration-300">
        <div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-4">
          <div>
            <h3 className="text-xl font-bold text-white">Data Daftar Tembusan</h3>
        ...
        <div className="space-y-3">
          {masterData.tembusans?.map((item: any, idx: number) => (
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 max-w-3xl mx-auto">
      <div className="mb-8 border-b border-slate-800 pb-4">
        <h3 className="text-xl font-bold text-white flex items-center gap-2"><Edit size={24} className="text-blue-400"/> Editor Daftar Tembusan</h3>
      </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">List Data Tembusan</p>
          <button onClick={() => setFormData({...formData, list: [...(formData.list || []), ""]})} className="bg-slate-800 border border-slate-700 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase flex items-center gap-1 transition-colors">
            <Plus size={12} /> Tambah Kolom
          </button>
        </div>
        <div className="space-y-3">
          {(!formData.list || formData.list.length === 0) && <p className="text-sm text-slate-500 italic">Belum ada tembusan.</p>}
          {formData.list?.map((t: string, i: number) => (
            <div key={i} className="flex gap-4 items-center bg-slate-900 border border-slate-800 p-2 pl-4 rounded-xl focus-within:border-blue-500/50 focus-within:ring-2 focus-within:ring-blue-900/50 transition-all">
              <span className="text-xs font-bold text-slate-500 w-4">{i+1}.</span>
              <input type="text" className="flex-1 bg-transparent text-sm font-semibold outline-none py-1 text-slate-200 placeholder:text-slate-600" value={t} placeholder="Masukkan instansi..." 
                onChange={e => { const newList = [...formData.list]; newList[i] = e.target.value; setFormData({...formData, list: newList}); }} 
              />
              <button onClick={() => { const newList = formData.list.filter((_:any, idx:number) => idx !== i); setFormData({...formData, list: newList}); }} className="bg-red-900/20 text-red-400 hover:bg-red-500 hover:text-white p-2 rounded-lg transition-colors">
                <Trash size={14} />
              </button>
            </div>
          ))}
